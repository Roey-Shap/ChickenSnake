...
class LineSegment():
    ...
    def draw(self, image: Image, relative_offset: tuple[int, int]):
        ...
        if self.is_symbol:
            ...
            draw_pip_color_background(c1, c2, absolute_pos, image, self.font)

            # Draw final symbol overlay
            symbol_string = ""
            if self.text.isdecimal() or strings_have_overlap("tx", self.text.lower()):
            ...
            ImageDraw.Draw(image).text(
                absolute_pos, symbol_string, self.color, self.font, spacing=self.spacing
            )
        else:
            ImageDraw.Draw(image).text(
                absolute_pos, self.text, self.color, self.font, spacing=self.spacing
            )

    @staticmethod
    def tokenize_card_text(raw_text: str) -> list[str]:
    ...
    def split_text_for_symbols(
        ...
        # print()
        tokenized_strings = LineSegment.tokenize_card_text(raw_text)
        line_segments = []
...
def draw_pip_color_background(c1: str, c2: str, 
    ...
    ImageDraw.Draw(image).text(
        pos, "K", color_code_map[c1.lower()], font
    )
    ImageDraw.Draw(image).text(
